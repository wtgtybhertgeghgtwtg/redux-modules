...
export default function defaultModuleCreator<S: Object, C: CanonicalTransformations<S>>(options: CanonicalCreateModuleOptions<S, C>): ReduxModule<S, $ObjMap<C, ExtractActionCreatorType>, $ObjMap<C, ExtractTypeType>> {
  const {initialState, name, transformations} = options;
  const actionCreators = Object.create(null);
  const types = Object.create(null);
  const reducerMap = new Map();
  forEach(transformations, ({reducer}: CanonicalTransformation<S, *, *>, actionName: string) => {
